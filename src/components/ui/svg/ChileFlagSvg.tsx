import React from "react";
import { type Tsvgprops } from "./types";
export const ChileFlagSvg: React.FC<Tsvgprops> = ({
  width = "24px",
  height = "24px",
  fill = "currentColor",
  className,
  ...props
}) => {
  return (
    <svg
      fill={fill}
      className={`transition-colors duration-1000 ${className}`}
      width={width}
      aria-label="Info Icon"
      height={height}
      x="0px"
      y="0px"
      viewBox="0 0 496 496"
      
      xmlSpace="preserve"
      {...props}
    >
      <path
        style={{
          fill: "#E13E3E",
        }}
        d="M0,249.2v121.6c0,27.2,21.6,46.4,48,46.4h400c26.4,0,48-19.2,48-46.4V249.2H0z"
      />
      <path
        style={{
          fill: "#D12A2A",
        }}
        d="M248,249.2l197.6,168c26.4,0,50.4-19.2,50.4-46.4V249.2H248z"
      />
      <path
        style={{
          fill: "#FFFFFF",
        }}
        d="M448,78.8H48c-26.4,0-48,21.6-48,48.8v121.6h496V127.6C496,100.4,474.4,78.8,448,78.8z"
      />
      <path
        style={{
          fill: "#E9F3F4",
        }}
        d="M448,78.8H48l200,170.4h248V127.6C496,100.4,474.4,78.8,448,78.8z"
      />
      <path
        style={{
          fill: "#3757A6",
        }}
        d="M48,81.2c-26.4,0-48,19.2-48,46.4v121.6h192v-168H48z"
      />
      <path
        style={{
          fill: "#C11414",
        }}
        d="M496,370c0,27.2-21.6,47.2-48,47.2H48c-26.4,0-48-20.8-48-48"
      />
      <path
        style={{
          fill: "#D9ECED",
        }}
        d="M48,78.8h400c26.4,0,48,21.6,48,48.8v88.8"
      />
      <polygon
        style={{
          fill: "#2A4D93",
        }}
        points="192,81.2 48,81.2 192,202 "
      />
      <path
        style={{
          fill: "#C11414",
        }}
        d="M496,370c0,29.6-21.6,47.2-48,47.2H48c-26.4,0-48-20.8-48-48"
      />
      <polygon
        style={{
          fill: "#19407F",
        }}
        points="192,81.2 48,81.2 192,122.8 "
      />
      <polygon
        style={{
          fill: "#FFFFFF",
        }}
        points="96,124.4 106.4,156.4 140,156.4 112.8,176.4 123.2,207.6 96,188.4 69.6,207.6 79.2,176.4  52.8,156.4 85.6,156.4 "
      />
      <polygon
        style={{
          fill: "#D9ECED",
        }}
        points="122.4,206 112.8,176.4 140,156.4 106.4,156.4 96,124.4 85.6,156.4 72.8,156.4 "
      />
    </svg>
  );
};
